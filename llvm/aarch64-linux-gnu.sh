...
if [ ! -f "$CURRENTTRIPLEPATH/zlib/include/zlib.h" ]; then
mkdir -p "$CURRENTTRIPLEPATH/zlib_build"
cd $CURRENTTRIPLEPATH/zlib_build
cmake -GNinja ${TOOLCHAINS_BUILD}/zlib_build -DCMAKE_SYSROOT=$SYSROOTPATH -DCMAKE_RC_COMPILER=llvm-windres \
	-DCMAKE_C_COMPILER=clang -DCMAKE_CXX_COMPILER=clang++ -DCMAKE_ASM_COMPILER=clang \
	-DCMAKE_INSTALL_PREFIX=$CURRENTTRIPLEPATH/zlib \
...
cmake $LLVMPROJECTPATH/llvm \
	...
	-DZLIB_INCLUDE_DIRS=$CURRENTTRIPLEPATH/zlib/include \
	-DHAVE_ZLIB=On \
	-DZLIB_LIBRARY=$CURRENTTRIPLEPATH/zlib/lib/libz.a \
