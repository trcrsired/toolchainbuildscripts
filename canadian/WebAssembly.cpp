...
void WebAssembly::AddClangCXXStdlibIncludeArgs(const ArgList &DriverArgs,
  ...
  case ToolChain::CST_Libcxx: {
    if (getTriple().getOS() != llvm::Triple::UnknownOS) {
      addSystemInclude(DriverArgs, CC1Args,
                       SysRoot + "/include/" + MultiarchTriple + "/c++/v1");
    }
  ...
  case ToolChain::CST_Libstdcxx: {
    ...
    std::string CxxIncludePath = SysRoot + "/include/c++/";
    // Walk the subdirs, and find the one with the newest gcc version:
    for (llvm::vfs::directory_iterator
             LI = getVFS().dir_begin(CxxIncludePath, EC),
             LE;
    ...
    if (Version.Major == -1)
      break;
    std::string LibstdcxxInclude = SysRoot + "/include/c++/" + Version.Text;
    addSystemInclude(DriverArgs, CC1Args,
                     LibstdcxxInclude + "/" + MultiarchTriple);
