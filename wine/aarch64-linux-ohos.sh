...
enable_wineandroid_drv=no CC=clang CXX=clang++ MINIMUMBUILD=yes DISABLEALSA=yes HOST=aarch64-linux-ohos ARCH=aarch64 CMAKEEXTRAFLAGS="-DCMAKE_CXX_FLAGS=\"-I$SYSROOT/usr/include/c++/v1\"" ./wine.sh "$@"