...
cd $TOOLCHAINS_BUILD/xorgproto
NOCONFIGURE=1 ./autogen.sh
if [ $? -ne 0 ]; then
echo "xorgproto autogen failed"
exit 1
fi
echo "$(date --iso-8601=seconds)" > ${TOOLCHAINS_BUILD}/xorgproto/.autogensuccess
fi


if [ ! -f $currentpath/xorgproto/.configuresuccess ]; then
mkdir -p $currentpath/xorgproto
cd $currentpath/xorgproto
STRIP=llvm-strip ${TOOLCHAINS_BUILD}/xorgproto/configure --disable-nls --disable-werror --host=$HOST --prefix=$currentpath/installs --cache-file=$currentpath/xorgproto/config.cache --enable-malloc0returnsnull 
if [ $? -ne 0 ]; then
echo "xorgproto configuresuccess failed"
exit 1
fi
echo "$(date --iso-8601=seconds)" > ${TOOLCHAINS_BUILD}/xorgproto/.configuresuccess
fi

if [ ! -f $currentpath/xorgproto/.buildsuccess ]; then
mkdir -p $currentpath/xorgproto
cd ${currentpath}/xorgproto
make -j$(nproc)
if [ $? -ne 0 ]; then
echo "xorgproto build failure"
exit 1
fi
echo "$(date --iso-8601=seconds)" > $currentpath/xorgproto/.buildsuccess
fi

if [ ! -f $currentpath/xorgproto/.installsuccess ]; then
cd ${currentpath}/xorgproto
make install -j$(nproc)
if [ $? -ne 0 ]; then
...
mkdir -p "$currentpath"
git pull --quiet
if [ ! -d "$TOOLCHAINS_BUILD/xauth" ]; then
cd "$TOOLCHAINS_BUILD"
...
if [ ! -f $TOOLCHAINS_BUILD/xauth/.autogensuccess ]; then
mkdir -p $TOOLCHAINS_BUILD/xauth
cd $TOOLCHAINS_BUILD/xauth
NOCONFIGURE=1 ./autogen.sh
if [ $? -ne 0 ]; then
echo "xauth autogen failed"
exit 1
fi
...
cd $currentpath/xauth
STRIP=llvm-strip ${TOOLCHAINS_BUILD}/xauth/configure --disable-nls --disable-werror --host=$HOST --prefix=$currentpath/installs --cache-file=$currentpath/xauth/config.cache --enable-malloc0returnsnull 
if [ $? -ne 0 ]; then
echo "xauth configuresuccess failed"
exit 1
fi
echo "$(date --iso-8601=seconds)" > ${TOOLCHAINS_BUILD}/xauth/.configuresuccess
fi

if [ ! -f $currentpath/xauth/.buildsuccess ]; then
mkdir -p $currentpath/xauth
cd ${currentpath}/xauth
make -j$(nproc)
if [ $? -ne 0 ]; then
echo "xauth build failure"
exit 1
fi
echo "$(date --iso-8601=seconds)" > $currentpath/xauth/.buildsuccess
fi

if [ ! -f $currentpath/xauth/.installsuccess ]; then
cd ${currentpath}/xauth
make install -j$(nproc)
if [ $? -ne 0 ]; then
echo "xauth install failure"
exit 1
fi
echo "$(date --iso-8601=seconds)" > $currentpath/xauth/.installsuccess
fi
...
STRIP=llvm-strip ${TOOLCHAINS_BUILD}/libx11/configure --disable-nls --disable-werror --host=$HOST --prefix=$currentpath/installs --cache-file=$currentpath/libx11/config.cache --enable-malloc0returnsnull 
if [ $? -ne 0 ]; then
