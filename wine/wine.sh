...
CONFIG_SITE=$currentpath/libx11/config.cache STRIP=llvm-strip ${TOOLCHAINS_BUILD}/libx11/configure --disable-nls --disable-werror --host=$HOST --prefix=$currentpath/installs --cache-file=$currentpath/libx11/config.cache
if [ $? -ne 0 ]; then
