...
if [[ "$target_os" == linux ]] || [[ "$target_os" =~ ^freebsd ]] ||
    ...
    # DJGPP doesn't support threads, libquadmath and libssp
    configures="$configures --disable-threads --disable-libquadmath --disable-libssp"
else
...
if [ ! -f "${build_prefix_project}/${build_phase_file}" ]; then
    ...
    if [ $? -ne 0 ]; then
        ...
        if [[ "x$project_name" == "xgcc" ]] && [[ "x$not_having_fenv_fix" == "xyes" ]]; then
            ...
            if [ -d "$libstdcpp_path" ]; then
                echo "Detected libstdc++-v3 directory, attempting standalone build..."

                # Try building libstdc++ alone
                cd "$libstdcpp_path"
                make -j "${JOBS}"
                if [ $? -ne 0 ]; then
                    echo "Standalone libstdc++ build failed, applying fenv.h workaround..."

                    # Path to config.h generated by libstdc++
                    local config_h="${libstdcpp_path}/config.h"
                    if [ -f "$config_h" ]; then
                        # Replace '#define HAVE_FENV_H 1' with '/* #undef HAVE_FENV_H */'
                        sed -i 's/#define HAVE_FENV_H 1/\/\* #undef HAVE_FENV_H \*\//' "$config_h"
                        echo "Applied fenv.h workaround to $config_h"
                    fi
...
packaging_toolchain() {
    ...
    if [ ! -f "$build_prefix/.packagesuccess" ]; then
        rm -f "${prefix}.tar.xz"
        cd "$prefix_parent"
        XZ_OPT=-e9T0 tar --hard-dereference -cJf ${host_triplet}.${target_triplet}.tar.xz ${target_triplet}
