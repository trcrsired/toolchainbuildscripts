...
CANADADIANHOST=x86_64-w64-mingw32
export PATH=$TOOLCHAINSPATH/$BUILD/$CANADADIANHOST/bin:$PATH
CANADIANHOSTPREFIX=$TOOLCHAINSPATH/$CANADADIANHOST/$HOST
CANADIANHOSTPREFIXTARGET=$CANADIANHOSTPREFIX
...
if [ ! -d ${currentpath}/installs/glibc ]; then
	...
	if [ ! -d $linuxkernelheaders ]; then
		...
		make headers_install ARCH=arm64 -j INSTALL_HDR_PATH=${currentpath}/install/glibc/canadian
	fi
...
if [ ! -f $HOSTPREFIXTARGET/include/stdio.h ]; then
	cp -r ${currentpath}/install/linux/* $HOSTPREFIXTARGET/
	cp -r ${currentpath}/install/glibc/canadian/* $HOSTPREFIXTARGET/
fi
...
mkdir -p ${currentpath}/hostbuild
mkdir -p ${currentpath}/hostbuild/$CANADIANHOST
cd ${currentpath}/hostbuild/$CANADIANHOST
mkdir -p ${currentpath}/targetbuild/$CANADIANHOST/binutils-gdb
cd ${currentpath}/targetbuild/$CANADIANHOST/binutils-gdb
if [ ! -f Makefile ]; then
$TOOLCHAINS_BUILD/binutils-gdb/configure --disable-nls --disable-werror --enable-gold $CANADIANTRIPLETTRIPLETS --prefix=$CANADIANHOSTPREFIX
fi
...
mkdir -p ${currentpath}/targetbuild/$CANADIANHOST/gcc
cd ${currentpath}/targetbuild/$CANADIANHOST/gcc
if [ ! -f Makefile ]; then
$TOOLCHAINS_BUILD/gcc/configure --with-gxx-libcxx-include-dir=$CANADIANHOSTPREFIXTARGET/include/c++/v1 --prefix=$CANADIANHOSTPREFIX $CANADIANTRIPLETTRIPLETS $GCCCONFIGUREFLAGSCOMMON
fi
...
cd $TOOLCHAINSPATH/$CANADIANHOST
if [ ! -f $HOST.tar.xz ]; then
	XZ_OPT=-e9T0 tar cJf $HOST.tar.xz $HOST
	chmod 755 $HOST.tar.xz
fi
